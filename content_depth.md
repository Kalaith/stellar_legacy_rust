...
| Axis | Current (updated 2026-07-22, after iteration 30) |
| --- | --- |
| Events | **171** total — legacy_drift 31, survival 24, engineering 23, biology_medical 16, diplomacy 14, ethics 13, mystery 13, comedy 13, exploration_first_contact 12, science_anomaly 12 (every family ≥12) |
| Voice | data-driven flavor: obituary/succession/coming-of-age (per-generation), ambient dead-air lines, occurrence-aware phase-transition lines, **homecoming prose keyed by success level** (complete/partial/pyrrhic/failure) |
...
| Charters | **15** in `assets/contracts.json`, all destination-`tags`-annotated; **5 shapes**: standard deep-survey, 2 long-station (parked on-site), double-hop (two Operation legs), return-dominant (320-yr homeward haul), **outbound-dominant** (300-yr one-way push, arrival is the reward) |
| Campaign beats | seeded skeleton (data-driven pools + **three-era layering: founding / deep-middle / homecoming**) + **drift-threshold beats** (legacy_drift as cultural_drift crosses 0.3/0.5/0.7/0.85) + **adaptation-threshold beats** (biology_medical as adaptation crosses 0.35/0.6/0.8) |
...
| Subsystems | **6** (medical, life-support, agriculture, security, education, engineering) with decay + knowledge-gated repair; events can `subsystem_deltas` condition/knowledge; **every module now has both a knowledge-crisis and a condition-breakdown beat** (data-load asserts both), plus data-expressed cross-couplings (agriculture→medical) |
